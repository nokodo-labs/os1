...
<div class="mx-auto w-full max-w-5xl px-6 pt-10 pb-24">
	...
	<div class="mt-6 space-y-3">
		{#if runs.length === 0 && !loading}
			<div class="rounded-xl border border-white/10 bg-white/5 p-5 text-sm text-white/40">
				no runs
			</div>
		{/if}
			...
			<div class="rounded-xl border border-white/10 bg-white/5 p-5">
				<div class="flex items-start justify-between gap-4">
					<div class="min-w-0 flex-1">
						...
						<div class="mt-1 space-y-0.5 text-xs text-white/50">
							<div>
								<span class="text-white/30">run:</span>
								<code class="text-white/60">{run.run_id}</code>
							</div>
							<div>
								<span class="text-white/30">thread:</span>
								<a
							...
							<div>
								<span class="text-white/30">agent:</span>
								<code class="text-white/60">{run.agent_id}</code>
							</div>
						</div>
					</div>

					<div class="flex shrink-0 flex-col items-end gap-1 text-xs text-white/40">
				...
				<div class="mt-3">
					<a
						href={resolve(`/c/${run.thread_id}`)}
						class="rounded-lg border border-white/10 bg-white/5 px-2.5 py-1 text-xs text-white/60 transition hover:bg-white/10"
					>
						open thread
					</a>
				</div>
	...
	<div class="mt-10">
		...
		<div class="mt-4 rounded-xl border border-white/10 bg-white/5 p-5">
			<div class="text-sm font-semibold text-white/85">service worker</div>
			<div class="mt-2 space-y-1 text-xs text-white/50">
				<div>
					<span class="text-white/30">status:</span>
					<span class="text-white/60">{swStatus}</span>
				</div>
				{#if swScriptUrl}
					<div>
						<span class="text-white/30">script:</span>
						<code class="break-all text-white/60">{swScriptUrl}</code>
					</div>
				{/if}
			</div>
		...
		<div class="mt-4 rounded-xl border border-white/10 bg-white/5 p-5">
			<div class="flex items-center justify-between">
				<div class="text-sm font-semibold text-white/85">cache storage</div>
				<button
					type="button"
					class="rounded-lg border border-red-500/20 bg-red-500/5 px-2.5 py-1 text-xs text-red-400/80 transition hover:bg-red-500/10"
			...
			{#if caches_.length === 0}
				<div class="mt-3 text-xs text-white/40">no caches found</div>
			{:else}
				<div class="mt-3 space-y-1.5">
					{#each caches_ as cache (cache.name)}
						<div
							class="flex items-center justify-between rounded-lg border border-white/5 bg-white/2 px-3 py-2 text-xs"
						>
							<code class="text-white/60">{cache.name}</code>
							<span class="text-white/40">{cache.count} entries</span>
						</div>
					{/each}
				</div>
			{/if}
