...
<div class="flex h-dvh w-full flex-col overflow-auto lg:flex-row">
	...
	>
		...
		<div class="absolute top-3 left-4 z-30">
			<div class="rounded-full bg-black/55 px-3 py-1.5 text-xs font-medium text-foreground/80">
		...
		<div class="relative z-10 flex h-full w-full flex-col gap-4 p-6 pt-14">
			<div
				class="liquid-metal flex items-center px-5 text-sm text-foreground/85"
			...
			<div
				class="liquid-metal px-5 py-4 text-sm text-foreground/82"
				style="width: {cardW}px; height: {cardH}px; border-radius: {cornerRadius}px; filter: url(#{cardFilterId});"
			>
				<div class="font-semibold text-foreground/92">metal card</div>
				<div class="mt-2 text-foreground/65">edge reflections only, opaque mirror surface</div>
				<div class="mt-4 flex gap-2">
					<div
						class="liquid-metal grid place-items-center text-xs text-foreground/90"
	...
	<section class="min-h-80 flex-1 bg-black/35 p-6">
		<div class="text-xs font-semibold tracking-wider text-foreground/60 uppercase">controls</div>
		<div class="mt-4 grid gap-5">
			<div>
				<div class="mb-2 text-xs text-foreground/65">bezel width: {bezelWidth}px</div>
				<Slider bind:value={bezelWidth} min={8} max={36} step={1} />
			</div>
			<div>
				<div class="mb-2 text-xs text-foreground/65">thickness: {thickness}px</div>
				<Slider bind:value={thickness} min={8} max={40} step={1} />
			</div>
			<div>
				<div class="mb-2 text-xs text-foreground/65">
					reflection warp: {refractionStrength.toFixed(2)}
				</div>
				<Slider bind:value={refractionStrength} min={0.6} max={2.2} step={0.01} />
			</div>
			<div>
				<div class="mb-2 text-xs text-foreground/65">mirror depth: {mirrorDepth}px</div>
			...
			<div>
				<div class="mb-2 text-xs text-foreground/65">specular angle: {specularAngle}deg</div>
