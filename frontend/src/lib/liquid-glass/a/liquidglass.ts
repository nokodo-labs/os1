...
const GLASS_CLASS = 'liquid-glass-action'
...
/**
 * turns any element into a glass surface.
 *
 ...
 * on hover / focus / active state changes. a preset swapped at runtime
 * re-runs the attachment, so the new one starts from its own base values.
 *
 * @example
 * ```svelte
 * <button {@attach liquidGlass('nav')} class="rounded-full p-3">click me</button>
 * ```
 */
export function liquidGlass(input: GlassInput): Attachment<HTMLElement> {
	return (node) => {
		...
		node.classList.add(GLASS_CLASS)
		applyProps(node, current, preset.base)
		...
		return () => {
			...
			node.classList.remove(GLASS_CLASS)
		}
	}
}
