...
class GroupsCache {
	...
	async update(id: string, params: GroupUpdate): Promise<Group | null> {
		const { data, error } = await apiClient().PATCH('/groups/{group_id}', {
	...
	async remove(id: string): Promise<boolean> {
		const { error } = await apiClient().DELETE('/groups/{group_id}', {
			params: { path: { group_id: id } },
		})
		return !error
	}
	...
	async addMember(
		...
		const { data, error } = await apiClient().POST('/groups/{group_id}/members', {
	...
	async removeMember(groupId: string, userId: string): Promise<boolean> {
		const { error } = await apiClient().DELETE('/groups/{group_id}/members/{user_id}', {
