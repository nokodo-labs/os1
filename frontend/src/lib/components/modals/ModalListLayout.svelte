...
<div class="flex flex-col gap-3">
	...
	<div class="flex items-center gap-2">
		<div class="relative flex-1">
			...
			<input
				type="text"
				class="rounded-pill w-full border border-foreground/10 bg-foreground/5 py-2 pr-3 pl-9 text-sm text-foreground/90 placeholder:text-foreground/40 transition-colors outline-none focus:border-foreground/20 focus:bg-foreground/8"
		...
		<select
			class="rounded-pill border border-foreground/10 bg-foreground/5 px-3 py-2 text-sm text-foreground/70 transition-colors outline-none focus:border-foreground/20 [&>option]:bg-neutral-900 [&>option]:text-foreground/90"
			value={sortIndex}
	...
	<div class="max-h-80 min-h-40 overflow-y-auto" onscroll={onScroll}>
		{#if loading}
			<div class="flex items-center justify-center py-12">
				<div
					class="h-5 w-5 animate-spin rounded-full border-2 border-foreground/20 border-t-white/60"
				></div>
			</div>
		{:else if isEmpty}
			<div class="rounded-container border border-foreground/8 bg-foreground/3 p-6">
				<p class="text-center text-sm text-foreground/40">
