...
<BaseToolCard {execution} {compact}>
	...
	{#snippet body()}
		{#if recallQuery}
			<div class="mb-3 text-sm text-foreground/60">
		...
		{#if memories.length > 0}
			<div class="space-y-2">
				{#each memories as memory, idx (idx)}
					<div class="rounded-pill bg-foreground/5 px-3 py-2 text-sm text-foreground/70">
						{typeof memory === 'string' ? memory : JSON.stringify(memory)}
					</div>
				{/each}
			</div>
		{:else if execution.status === 'completed' && !execution.result?.isError}
			<div class="text-sm text-foreground/50">no memories found</div>
		{/if}
