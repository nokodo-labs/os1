...
{#if compact}
	<span
		class="inline-flex items-center gap-1.5 text-xs text-white/60"
		in:fade={{ duration: 150 }}
	>
		<Brain class="h-3.5 w-3.5" />
		{#if isActive}
			<ShimmerText>thinking</ShimmerText>
			<span class="text-white/50 tabular-nums">{elapsed.toFixed(1)}s</span>
		{:else if isDone}
	...
	<div class="flex items-center gap-2.5 py-1.5" in:fade={{ duration: 150 }}>
		...
		<div class="flex items-center gap-2 text-sm">
			{#if isActive}
				...
				<span class="text-xs text-white/50 tabular-nums">{elapsed.toFixed(1)}s</span>
			{:else if isDone}
