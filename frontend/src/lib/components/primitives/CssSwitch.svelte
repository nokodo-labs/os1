<script lang="ts">
	...
	const SIZES = {
		sm: { thumb: { width: 48, height: 30 }, slider: { width: 60, height: 24 } },
		md: { thumb: { width: 73, height: 46 }, slider: { width: 80, height: 34 } },
		lg: { thumb: { width: 109, height: 69 }, slider: { width: 120, height: 50 } },
		xl: { thumb: { width: 146, height: 92 }, slider: { width: 160, height: 67 } },
	} as const
	...
	const thumbX = $derived(() => {
		if (isDragging) return Math.max(0, Math.min(1, dragRatio)) * TRAVEL
