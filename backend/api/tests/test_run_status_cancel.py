...
async def test_attach_task_and_cancel_run_stops_task() -> None:
	...
	await store.attach_task("run_test_1", task)

	# cancel via store API
	cancelled = await store.cancel_run("run_test_1")
...
async def test_cancel_run_returns_false_when_no_task() -> None:
	...
	store = RunStatusStore()
	# unknown run
	assert await store.cancel_run("run_does_not_exist") is False
	...
	assert await store.cancel_run("run_test_2") is False
...
async def test_cancel_run_returns_false_when_task_already_done() -> None:
	...
	await task
	await store.attach_task("run_test_3", task)

	assert await store.cancel_run("run_test_3") is False
...
async def test_subscriber_disconnect_does_not_kill_run() -> None:
	...
	await store.publish("run_test_4", b"event: delta\ndata: {}\n\n")

	result = await store.subscribe("run_test_4")
	assert result is not None
	catchup, q = result
	assert len(catchup) == 1

	# subscriber disconnects
	await store.unsubscribe("run_test_4", q)

	# run is still alive in the store
	assert await store.get_run("run_test_4") is not None

	# new subscriber can still attach and gets the same catchup log
	result2 = await store.subscribe("run_test_4")
