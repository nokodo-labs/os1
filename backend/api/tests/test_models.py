"""Model tests."""

from __future__ import annotations

import pytest
from fastapi import HTTPException
from httpx import AsyncClient
from sqlalchemy.ext.asyncio import AsyncSession

from api.models.model import ModelType
from api.schemas.model import ModelCreate
from api.schemas.provider import ProviderCreate
from api.v1.service import models as model_service
from api.v1.service import providers as provider_service


@pytest.mark.asyncio
async def test_create_model(db_session: AsyncSession) -> None:
	# Create provider
	provider = await provider_service.create_provider(
		ProviderCreate(name="P_Model", adapter_type="openai"), db_session
	)

	model_in = ModelCreate(
		provider_id=provider.id,
		name="gpt-4",
		model_type=ModelType.LLM,
		capabilities=["chat"],
		enabled=True,
	)
	model = await model_service.create_model(model_in, db_session)
	assert model.name == "gpt-4"
	assert model.provider_id == provider.id


@pytest.mark.asyncio
async def test_create_model_invalid_provider(db_session: AsyncSession) -> None:
	model_in = ModelCreate(
		provider_id="nonexistent",
		name="gpt-4",
		model_type=ModelType.LLM,
		capabilities=[],
		enabled=True,
	)
	with pytest.raises(HTTPException) as exc:
		await model_service.create_model(model_in, db_session)
	assert exc.value.status_code == 404


@pytest.mark.asyncio
async def test_list_models(db_session: AsyncSession) -> None:
	provider = await provider_service.create_provider(
		ProviderCreate(name="P_List", adapter_type="openai"), db_session
	)
	model_in = ModelCreate(
		provider_id=provider.id,
		name="m1",
		model_type=ModelType.LLM,
		capabilities=[],
		enabled=True,
	)
	await model_service.create_model(model_in, db_session)

	models = await model_service.list_models(db_session)
	assert len(models) >= 1

	models_filtered = await model_service.list_models(
		db_session, provider_id=provider.id
	)
	assert len(models_filtered) >= 1

	models_empty = await model_service.list_models(
		db_session, provider_id="nonexistent"
	)
	assert len(models_empty) == 0


@pytest.mark.asyncio
async def test_get_model(db_session: AsyncSession) -> None:
	provider = await provider_service.create_provider(
		ProviderCreate(name="P_Get", adapter_type="openai"), db_session
	)
	model_in = ModelCreate(
		provider_id=provider.id,
		name="m2",
		model_type=ModelType.LLM,
		capabilities=[],
		enabled=True,
	)
	created = await model_service.create_model(model_in, db_session)

	fetched = await model_service.get_model(created.id, db_session)
	assert fetched.id == created.id


@pytest.mark.asyncio
async def test_get_model_not_found(db_session: AsyncSession) -> None:
	with pytest.raises(HTTPException) as exc:
		await model_service.get_model("nonexistent", db_session)
	assert exc.value.status_code == 404


@pytest.mark.asyncio
async def test_models_router_endpoints(client: AsyncClient) -> None:
	"""Hit list and detail routes to cover router branch logic."""
	provider_resp = await client.post(
		"/v1/providers",
		json={"name": "models-router-provider", "adapter_type": "openai"},
	)
	assert provider_resp.status_code == 201
	provider_id = provider_resp.json()["id"]
	model_resp = await client.post(
		"/v1/models",
		json={
			"provider_id": provider_id,
			"name": "models-router",
			"model_type": "llm",
			"capabilities": ["chat"],
		},
	)
	assert model_resp.status_code == 201
	model_id = model_resp.json()["id"]

	list_resp = await client.get("/v1/models")
	assert list_resp.status_code == 200
	assert any(item["id"] == model_id for item in list_resp.json())

	detail_resp = await client.get(f"/v1/models/{model_id}")
	assert detail_resp.status_code == 200
	assert detail_resp.json()["id"] == model_id
