...
async def run_agent(
	...
	*,
	...
	The function creates its own database session so the streaming generator
	is fully decoupled from the request-scoped ``get_db()`` dependency.
	"""
	...
	async with AsyncSessionLocal() as session:
		...
		# register run in status store + broadcast run.started to participants
		...
		if input is not None and input.strip():
			...
			# track user message in run status
		...
		try:
			...
			# Restore original head to avoid unintended side effects
			if initial_parent_id and original_head != initial_parent_id:
		...
		try:
			...
			async for delta in stream:
				...
				if delta.chat is not None:
					...
					# update run status with streaming content
					streaming_text = ""
	...
	# fire-and-forget metadata generation. generate_thread_metadata creates
	# its own session when called without one and resolves the task model
	# from settings.
	_track_async_task(
